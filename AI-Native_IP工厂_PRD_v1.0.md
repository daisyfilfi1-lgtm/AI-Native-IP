...
class IPAsset:
    asset\\\_id: str
    ip\\\_id: str
    asset\\\_type: story|opinion|joke|data
    content: str
    content\\\_vector: List\\\[float]
    metadata: {
        emotion\\\_tags, scene\\\_tags, cognitive\\\_tags,
        product\\\_relevance, usage\\\_count, core\\\_asset
    }
...
class ContentDraft:
    ...
    level: A|B|C
    workflow: {
        source\\\_topic\\\_id, competitor\\\_urls,
        remix\\\_version, generation\\\_model, assets\\\_used
    }
    quality\\\_score: {
        originality, ip\\\_similarity,
        emotion\\\_curve, predicted\\\_completion
    }
    compliance\\\_status: pending|passed|failed|warning
```
...
Request: {
    workflow\\\_type: daily\\\_content,
    ip\\\_id: zhangkai\\\_001,
    content\\\_level: B,
    priority: normal
}
Response: {
...
```
GET /api/v1/orchestrator/task/{task\\\_id}/status
```
...
Request: {
    ip\\\_id: zhangkai\\\_001,
    query: 深夜崩溃但最终翻盘的故事,
    filters: {emotion\\\_tags: \\\[despair, hope], usage\\\_limit: 3}
}
```
...
Request: {
    remix\\\_version\\\_id: remix\\\_v1\\\_anger,
    ip\\\_id: zhangkai\\\_001,
    content\\\_level: B,
    style\\\_strength: 0.8
}
Response: {
    draft\\\_id: draft\\\_0317\\\_01,
    full\\\_script: 完整口播文案,
    emotion\\\_curve: \\\[...],
    assets\\\_cited: \\\[story\\\_001, opinion\\\_003]
}
```
...
1. **模型幻觉**：强制RAG检索，素材必须带source\_id
2. **平台限流**：保留真人内容占比>30%
