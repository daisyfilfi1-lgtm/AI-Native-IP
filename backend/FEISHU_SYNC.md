...
| 现象 | 可能原因 | 处理 |
|------|----------|------|
| 503 请先填写 App ID / App Secret | 凭证未配置 | 在管理后台或环境变量中配置 `FEISHU_APP_ID`、`FEISHU_APP_SECRET` |
| 502 飞书 API 调用失败 | token、网络或权限问题 | 检查开放平台凭证、网络连通性，确认应用权限已开通 |
| 502 获取失败 code=99xxx / 131006 | 应用未加入知识库成员 | 知识库 → 设置 → 成员与权限 → 添加成员 → 选择该应用 |
| 502 获取失败 code=91403/95008/95009 | 无文档读权限 | 在开放平台为应用开通云文档只读或「查看、评论和下载云空间中所有文件」权限 |
| 未获取到任何知识空间 | 应用无知识库权限或未加入任何空间 | 确认 `wiki:space:read` 等权限已开通，并将应用添加为目标知识库成员 |
